...
const WeeklyView = ({ courses, trainers, setCourses }) => {
  ...
  // ✅ FIX v2.12.1: Activities für einen Tag holen - robusteres Date-Matching
  const getActivitiesForDay = (dayIndex) => {
    ...
    return weeklyActivities.filter(activity => {
      // Prüfe verschiedene mögliche Datums-Felder
      const activityDate = activity.date || activity.activity_date || activity.activityDate;
      if (!activityDate) {
        // Fallback: Prüfe day_of_week wenn kein Datum vorhanden
        const dayNames = ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag'];
        return activity.day_of_week === dayNames[dayIndex] || activity.dayOfWeek === dayNames[dayIndex];
      }
      
      const actDateStr = activityDate.split('T')[0];
  ...
  useEffect(() => {
    ...
    // ✅ FIX v2.12.1: Sonderaktivitäten laden - IMMER, auch bei Ferien
    const fetchWeeklyActivities = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.length > 0) {
            console.log('📋 Activities Details:', data.map(a => ({
              id: a.id,
              title: a.title,
          ...
          console.warn(`⚠️ Activities API returned ${response.status}`);
        }
      } catch (error) {
        ...
        {isHolidayWeek() && (
          <div className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              {/* ✅ FIX v2.12.1: Zeige Activities-Hinweis auch bei Ferien */}
              {weeklyActivities.length > 0 && (
                <div className="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-medium">
                  🎯 {weeklyActivities.length} Sonderaktivität{weeklyActivities.length !== 1 ? 'en' : ''}
                </div>
              )}
            </div>
          </div>
